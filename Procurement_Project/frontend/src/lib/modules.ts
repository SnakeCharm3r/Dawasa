...
export const modules: Record<string, ModuleConfig> = {
  ...
  quotations: {
    ...
    actions: [
      ...
      { label: "Send for requester review", tone: "primary", inputLabel: "Reason for selecting this proforma", requiredInput: true, path: (item) => `admin/supplier-quotations/${id(item)}/request-approval`, body: (input) => ({ reason_for_selection: input }), appliesTo: (item) => status(item) === "active" && !relatedId(item, "approval_recommendation") && ["approved_for_sourcing", "quotations_ready", "returned_to_sourcing"].includes(relatedStatus(item, "requisition")), visible: (item, user) => role(user, "super_admin", "procurement_officer") },
      { label: "Requester approve", tone: "primary", inputLabel: "Approval comments", path: (item) => `admin/quotation-recommendations/${relatedId(item, "approval_recommendation")}/requester-submit`, body: (input) => ({ comments: input }), appliesTo: (item) => status(item) === "active" && Boolean(relatedId(item, "approval_recommendation")) && relatedStatus(item, "requisition") === "pending_requester_approval", visible: (item, user) => role(user, "requester") && Number((item.requisition as JsonRecord | undefined)?.requester_id) === user.id },
      { label: "Requester return", prompt: "Return this proforma to sourcing?", inputLabel: "Required corrections", path: (item) => `admin/quotation-recommendations/${relatedId(item, "approval_recommendation")}/requester-return`, body: (input) => ({ comments: input }), appliesTo: (item) => status(item) === "active" && Boolean(relatedId(item, "approval_recommendation")) && relatedStatus(item, "requisition") === "pending_requester_approval", visible: (item, user) => role(user, "requester") && Number((item.requisition as JsonRecord | undefined)?.requester_id) === user.id },
      { label: "Line manager approve", tone: "primary", inputLabel: "Approval comments", path: (item) => `admin/quotation-recommendations/${relatedId(item, "approval_recommendation")}/line-manager-approve`, body: (input) => ({ comments: input }), appliesTo: (item) => status(item) === "active" && Boolean(relatedId(item, "approval_recommendation")) && relatedStatus(item, "requisition") === "pending_line_manager_approval", visible: (item, user) => role(user, "line_manager", "department_head") && Number((item.requisition as JsonRecord | undefined)?.line_manager_id) === user.id },
      { label: "Line manager return", prompt: "Return this to requester for review?", inputLabel: "Required corrections", path: (item) => `admin/quotation-recommendations/${relatedId(item, "approval_recommendation")}/line-manager-return`, body: (input) => ({ comments: input }), appliesTo: (item) => status(item) === "active" && Boolean(relatedId(item, "approval_recommendation")) && relatedStatus(item, "requisition") === "pending_line_manager_approval", visible: (item, user) => role(user, "line_manager", "department_head") && Number((item.requisition as JsonRecord | undefined)?.line_manager_id) === user.id },
      { label: "Approve proforma", tone: "primary", inputLabel: "Approval comments", path: (item) => `admin/quotation-recommendations/${relatedId(item, "approval_recommendation")}/approve`, body: (input) => ({ comments: input }), appliesTo: (item) => relatedStatus(item, "approval_recommendation") === "submitted" && relatedStatus(item, "requisition") === "pending_final_approval", visible: (item, user) => role(user, "gm") },
      { label: "Return", inputLabel: "Required corrections", path: (item) => `admin/quotation-recommendations/${relatedId(item, "approval_recommendation")}/return`, body: (input) => ({ comments: input }), appliesTo: (item) => relatedStatus(item, "approval_recommendation") === "submitted" && relatedStatus(item, "requisition") === "pending_final_approval", visible: (item, user) => role(user, "gm") },
      { label: "Reject", tone: "danger", inputLabel: "Reason for rejection", path: (item) => `admin/quotation-recommendations/${relatedId(item, "approval_recommendation")}/reject`, body: (input) => ({ comments: input }), appliesTo: (item) => relatedStatus(item, "approval_recommendation") === "submitted" && relatedStatus(item, "requisition") === "pending_final_approval", visible: (item, user) => role(user, "gm") },
  ...
  suppliers: {
    ...
    columns: [
      ...
      { label: "Code", key: "code" },
      { label: "Compliance", key: "compliance_status", format: "status" },
      { label: "Grade", key: "current_performance.grade", format: "status" },
      { label: "Overall score", key: "current_performance.overall_score" },
      { label: "On-time delivery", key: "current_performance.delivery_score" },
      { label: "Open incidents", key: "open_incidents_count" },
      { label: "Award eligibility", key: "award_eligibility", format: "status" },
    ],
    actions: [
      { label: "Verify & activate", tone: "primary", method: "PATCH", inputLabel: "KYC activation justification", requiredInput: true, path: (item) => `admin/suppliers/${id(item)}/activate`, body: (input) => ({ reason: input }), visible: (item, user) => item.is_active === false && role(user, "super_admin", "procurement_officer", "gm", "ceo") },
      { label: "Suspend", tone: "danger", method: "PATCH", prompt: "Suspend this supplier from sourcing?", inputLabel: "Reason for suspension", requiredInput: true, path: (item) => `admin/suppliers/${id(item)}/deactivate`, body: (input) => ({ reason: input }), visible: (item, user) => item.is_active === true && role(user, "super_admin", "gm", "ceo") },
    ],
  },
  entities: {
