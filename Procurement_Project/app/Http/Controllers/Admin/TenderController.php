...
class TenderController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = Tender::with(['category:id,name,code', 'requisition:id,requisition_number,status']);
    ...
    {
        $this->internal($request);
        return response()->json(['data' => $tender->load(['items', 'category', 'requisition:id,requisition_number,status,required_date', 'invitedSuppliers:id,name,code,portal_status'])]);
    }

    public function update(Request $request, Tender $tender): JsonResponse
    {
        $this->internal($request); abort_unless($tender->status === Tender::STATUS_DRAFT, 422, 'Only draft tenders can be edited.');
    ...
    {
        $this->internal($request); abort_unless($tender->status === Tender::STATUS_DRAFT, 422);
    ...
    {
        $this->internal($request); abort_unless($tender->status === Tender::STATUS_PUBLISHED, 422);
        $tender->update(['status' => Tender::STATUS_CLOSED]);
        return response()->json(['message' => 'Tender closed.']);
    }
    ...
    public function cancel(Request $request, Tender $tender): JsonResponse
    {
        ...
        $old = $tender->status; $tender->update(['status' => Tender::STATUS_CANCELLED, 'cancellation_reason' => $data['reason']]);
    ...
    {
        $this->internal($request); abort_if($tender->status === Tender::STATUS_PUBLISHED && $tender->submission_deadline->isFuture(), 403, 'Responses remain sealed until the deadline.');
    ...
    private function internal(Request $request): void { abort_unless($request->user()->hasAnyRole(['super_admin', 'procurement_officer', 'gm', 'ceo']), 403); }
