...
class SupplierVerificationController extends Controller
{
    ...
    public function decision(Request $request, Supplier $supplier): JsonResponse
    {
        ...
        if (in_array($data['decision'], ['approved', 'reactivated'], true)) {
            $assessment = $this->compliance->assess($supplier, false);
            abort_if($assessment['missing_documents'] || $assessment['expired_documents'] || $assessment['rejected_documents'], 422, 'All mandatory supplier documents must be verified and valid before approval.');
        }
        ...
        ActivityLog::record($request->user(), 'supplier.'.$data['decision'], $supplier, ['portal_status' => $old], ['portal_status' => $newStatus, 'comments' => $data['comments']]);
        return response()->json(['message' => 'Supplier decision recorded.', 'data' => $supplier->fresh()]);
    }
    ...
    {
        abort_unless($request->user()->hasAnyRole(['super_admin', 'procurement_officer', 'gm', 'accountant', 'ceo']), 403);
