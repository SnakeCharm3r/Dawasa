...
class UpdateUserRequest extends CeoAwareFormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($lineManagerId) {
                ...
                if ($lineManager && $updatedUser && $lineManager->id === $updatedUser->id) {
                ...
                if ($lineManager && $updatedUser && $this->createsCircularReporting($lineManager, $updatedUser)) {
                    $validator->errors()->add('line_manager_id', 'The selected line manager would create a circular reporting relationship.');
                }
            }
        });
    }

    private function createsCircularReporting(User $lineManager, User $user): bool
    {
        foreach ($user->directReports as $report) {
            if ($report->id === $lineManager->id) {
                return true;
            }

            if ($this->createsCircularReporting($lineManager, $report)) {
                return true;
            }
        }
